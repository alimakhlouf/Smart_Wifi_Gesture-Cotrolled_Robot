/* ----------------------------------------------------------------------------
[FILE NAME]: millis_mcu_config.h

[AUTHOR]: Ali Makhlouf

[DATE CREATED]: 12/5/2017

[DESCRIPTION]:  a library header file for delay using timer either ,it can also
                monitor a specified event or code during the delay and if the 
				action happened it jumps out of the delay
--------------------------------------------------------------------------------------------------- */
...
/*
it takes the same argument of millis(uint16 t) which holds the needed delay 
time we multiply the delay time by 1000 to convert it to microseconds
*/
#define DELAY_TIME_IN_US(t) ((unint32_t)(t) * 1000) 

#if defined(ATMEGA8) || defined(ATMEGA16) || defined(ATMEGA32)

//the following configurations is related to the timer in microseconds
 /* the time between each timer clock tick and the next one in
	microseconds (the same meaning as the time of 1024 cycles in
	microseconds because we are choosing 1024 prescaler)
	*/
#define TICK_TIME_1024_PRESCALER (((unint32_t)1024 * 1000000 / F_CPU))
...
#ifdef ATMEGA8
//define CONFIG_TMR0_OVF_ISR as the interrupt handler
