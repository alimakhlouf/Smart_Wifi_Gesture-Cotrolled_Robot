...
//NOTE any esp response is going to be polling so I have to disable the receive interrupt to prevent any received interrupt messages
//except the response to prevent data collision .. for the sending it's OK because any received data will be in a queue waiting to be
//processed not directly . but the problem is that I know that after sending there will be response so mostly if data is being 
//received while sending a message it will be neglected .. so I have to stop sending while data is being received--knowing data is being received or not is by checking
//by the
//receive flag becoming -1 or higher--(the esp waits before sending the \n and check if it's receiving any data then if it's then 
...
#if 0
	while (false == usart_is_data_ready(USART0))
	{
