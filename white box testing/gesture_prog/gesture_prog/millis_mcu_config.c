...
[DESCRIPTION]:a library c header file for delay using timer either ,it can also
			  monitor a specified event or code during the delay and if the
			  action happened it jumps out
----------------------------------------------------------------------------*/
...
bool millis(unint16_t t)
{
	...
	#endif
	//here we have to poll the flag .. if set check for each state of g_ovf_count 
	#if(MILLIS_INTERRUPT == 0) 
